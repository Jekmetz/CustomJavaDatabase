...
public class HashMap<K,V> implements Map<K,V>{
	...
	private Node<MapEntry<K,V>>[] data = null;			//data object
	...
	public boolean containsKey(Object key) {
		Node<MapEntry<K,V>> head = data[Math.floorMod(localHash(key),data.length)];	//Head is the data element that will be accepting the new Entry
		...
		{
			if(cur.data.getKey().equals(key))	//If the key is in that position...
				found = true;
	...
	public V get(Object key) {
		Node<MapEntry<K,V>> head =data[Math.floorMod(localHash(key),data.length)]; //set the head to the element in which that object will fall...
		...
		if(this.containsKey(key))	//If the key exists in the structure...	//could write the code for this myself again and modify to fit my needs but it is constant time so I don't care!
		{
			...
			{
				if(cur.data.getKey().equals(key))	//If we have found the node that has this key...
				{
					output = cur.data.getValue();	//give it to me!
	...
	public V put(K key, V value) {
		...
		Node<MapEntry<K,V>> head = data[Math.floorMod(localHash(key),data.length)];
		...
		doAResizeMate = calculateAlpha() >= 10; //Calculate alpha every time
	...
	public V remove(Object key) {
		...
		if(this.containsKey(key)) //If this contains the key to remove...
		{
			...
			while(!cur.next.data.getKey().equals(key))	//traverse the list until we are just before the one to remove
				cur = cur.next;
			
			value = cur.next.data.getValue();
	...
	private void resize()
	{
		...
		Node<MapEntry<K,V>>[] temp = data;
		data = (arrLengthIndex < arrLengths.length) ? new Node[arrLengths[arrLengthIndex]] : new Node[data.length*2+1];
		arrLengthIndex++;
		...
		
		{
			...
			{
				cur = temp[i];	//cur is the next head node
	...
	public String toString()
	{
		...
		{
			head = data[i];	//set the head for that element
	...
	public boolean equals(Object a)
	{
		...
		if(a instanceof adt.HashMap)	//If a is an adt.HashMap...
		{
			...
			if(this.size() == copy.size())	//If the sizes are the same...
				output = this.entrySet().equals(copy.entrySet());				
		}
	...
	private abstract class ViewIterator<T> implements Iterator<T> {
		int index = 0;
		int itemsViewed = 0;
		Node<MapEntry<K,V>> cur = data[0];
		Node<MapEntry<K,V>> next = null;
